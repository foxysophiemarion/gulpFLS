...
function gridKeywords() {
	/*
		  ...
		  data-grid-keywords-delay='500' - Задержка между изманой элементов. Указывать в ms 1s = 1000 (По умолчанию 500)
		  data-grid-keywords-direction='top' - Изменение направления анимации вверх. (По умолчанию bottom)
		  data-grid-keywords-ease="easeOut" - Тип анимации "easeOut", "easeIn", 'easeInOut. (По умолчанию easeOut)
	  */
	...
	$wrappers.forEach(($wrapper) => {
		...
		const updateTextContent = () => {
			...
			if ($sizer) {
				...
				const init = () => {
					// Помещаем все элементы в исходное положение и запускаем анимацию.
